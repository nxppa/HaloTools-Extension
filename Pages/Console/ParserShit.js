...
const Commands = {
    ...
    "export": {
        ...
        Executable: function (result) {
            ...
            if (localDataParsed && localDataParsed.Targets) {
                let outputArray = [];
                for (const walletAddress in localDataParsed.Targets) {
                    const walletObj = localDataParsed.Targets[walletAddress];
                    const alias = walletObj.Alias || "";
                    outputArray.push(`${walletAddress}:${alias}`);
                }
                const exportString = outputArray.join(';');
                Display(exportString);
            } else {
                Display("No wallets found to export");
            }
        },
